...
/** Phase A tests for [[ServerRenderer]].
  ...
  */
class ServerRendererSpec extends FunSuite:
  ...
  test("enterComponent within limit succeeds") {
    val r = ServerRenderer(ServerRenderer.Config(maxComponentDepth = 1000))
    (1 to 999).foreach(_ => r.enterComponent("ok"))
  }

  test("enterComponent beyond limit throws RenderException") {
    val r = ServerRenderer(ServerRenderer.Config(maxComponentDepth = 3))
    r.enterComponent("A")
    r.enterComponent("B")
    r.enterComponent("C")
    val e = intercept[RenderException] {
      r.enterComponent("D")
    }
  ...
  test("exitComponent decrements the counter") {
    val r = ServerRenderer(ServerRenderer.Config(maxComponentDepth = 2))
    r.enterComponent("A")
    r.enterComponent("B")
    r.exitComponent()
  ...
  test("push within size limit succeeds") {
    val r = ServerRenderer(ServerRenderer.Config(maxOutputBytes = 1024))
    r.push("x" * 500) // 1000 bytes (UTF-16 approx)
  }

  test("push beyond size limit throws RenderException") {
    val r = ServerRenderer(ServerRenderer.Config(maxOutputBytes = 1024))
    val e = intercept[RenderException] {
  ...
  test("body + head counted against the same limit") {
    val r = ServerRenderer(ServerRenderer.Config(maxOutputBytes = 100))
    r.push("a" * 20)      // 40 bytes
    r.head.push("b" * 20) // 40 bytes → total 80
    intercept[RenderException] {
