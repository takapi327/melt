...
/** Common supertype for [[State]][A] and [[Signal]][A].
  *
  ...
  *
  * This trait is intentionally not `sealed`: its subtypes ([[State]] and
...
/** Extracts the value type `A` from a [[ReactiveValue]][A].
  ...
  * tuple of reactive values `(State[A], Signal[B], ...)` to its value types
