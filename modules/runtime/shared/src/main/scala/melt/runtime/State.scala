...
import melt.runtime.impl.StateFactory
...
/** A mutable reactive variable — the common API contract used by both the
  ...
  * [[melt.runtime.impl.StateFactory]]:
  *
  ...
  *
  * User and generated code reference `State` through the shared factory, which
  * dispatches to the correct implementation automatically:
  *
  * {{{
  * val count   = State(0)         // JVM → JvmState; JS → JsState
...
/** Shared factory. Delegates to the platform-provided
  * [[melt.runtime.impl.StateFactory]] so that `State(0)` constructs a JVM no-op
  * or a Scala.js reactive cell automatically.
  */
object State:
  def apply[A](initial: A): State[A] = StateFactory.create(initial)

  /** Implicit conversion that allows a `State[A]` to be used directly as an
    ...
    * }}}
    */
  given [A]: Conversion[State[A], A] = _.value
