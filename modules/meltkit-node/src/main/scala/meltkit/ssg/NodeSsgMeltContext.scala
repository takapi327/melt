...
/** Node.js SSG-specific [[ServerMeltContext]] implementation.
  *
  * Runs on Node.js at build time — no live HTTP request exists.
  * [[render]] assembles the full HTML page via [[Template]] and stores it in
  * [[capturedHtml]] for [[NodeSsgGenerator]] to write to disk.
  *
  * Uses the Node.js [[Router]] (backed by `AsyncLocalStorage`) to set the
  * current request path, which behaves synchronously when the callback is synchronous.
  *
  * @tparam F the effect type
  * @tparam P the named-tuple of typed path parameters
  * @tparam B the request body type (`Unit` = no body)
  */
final class NodeSsgMeltContext[F[_], P <: AnyNamedTuple, B](
