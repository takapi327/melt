...
/** Configuration for a Node.js static site generation run.
  *
  * Uses `String` paths instead of `java.nio.file.Path` because Scala.js does not
  * provide the `java.nio.file` API.
  *
