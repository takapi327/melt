      ...
      case Success(response) =>
        try
          ...
          if isHead then
            ...
            os.close()
          exchange.close()
        catch case _: Throwable => try exchange.close() catch case _: Throwable => ()
  ...
  private def tryServeStaticFile(pathname: String, exchange: HttpExchange, isHead: Boolean): Boolean =
    config.clientDistDir match
      ...
      case Some(distDir) =>
        ...
        val ext         = Option(filePath.getFileName.toString).flatMap(n => Option(n.lastIndexOf('.')).filter(_ >= 0).map(i => n.substring(i))).getOrElse("")
