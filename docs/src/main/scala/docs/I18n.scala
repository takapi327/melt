...
object I18n:
  ...
  val en: Dict = Dict(
    ...
    exampleLinks = SidebarLinks(Map(
      "counter"  -> "Counter",
      "todo-app" -> "Todo App"
    )),
    home = Home(
      tagline        = "A single-file component framework for Scala.js. Compile Scala, HTML and CSS to lean DOM code — no runtime framework.",
      cta1           = "Get Started",
      cta2           = "View on GitHub",
      meta           = List("v0.1.0-SNAPSHOT", "Apache 2.0", "Scala 3.3.7"),
      featuresLabel  = "Why Melt",
      featuresTitle  = "The compiler is the framework.",
      featuresSub    = "Inspired by Svelte. Powered by the Scala type system. Targeting browsers, servers and statically-generated sites from the same .melt source.",
      features       = List(
        HomeFeature("01", "Single-file components",    "Write Scala, HTML and scoped CSS in one .melt file. The compiler emits direct DOM code — no virtual DOM, no runtime overhead."),
        HomeFeature("02", "Fully typed templates",     "Every expression in your template is checked by scalac. Refactors propagate; typos fail at compile time, not on a Tuesday in production."),
        HomeFeature("03", "Tiny reactive runtime",     "Just Var, Signal and Bind. Fine-grained updates, no re-rendering trees, no diffing. Effects and memos compose naturally."),
        HomeFeature("04", "SPA · SSR · SSG, one source", "The same component renders on the JVM as an HTML string or in the browser as live DOM. Hydration is opt-in.")
      ),
      demoLabel      = "Try it",
  ...
  val ja: Dict = Dict(
    ...
    home = Home(
      ...
      features       = List(
        HomeFeature("01", "シングルファイル構成",           "Scala・HTML・スコープ付き CSS を 1 つの .melt ファイルに記述。コンパイラが直接 DOM コードを生成し、仮想 DOM もランタイムオーバーヘッドもありません。"),
