...
/** Reads and parses the `-- MELT GENERATED --` source-map comment block
  * that melt appends to every generated `.scala` file.
  *
  * Block format:
  * {{{
  * /*
...
object MeltGeneratedSource:
  ...
  def read(generatedFile: File): Option[Meta] =
    if !generatedFile.exists() then None
    else
      val contentOpt =
        try
  ...
  /** Parses the source-map block from a generated Scala source string.
    ...
    * `LINES:` format for files generated by older melt versions.
    */
  def parse(content: String): Option[Meta] =
    val blockEnd   = content.lastIndexOf(BlockStart)
    val blockStart = if blockEnd >= 0 then content.lastIndexOf(BlockStart, blockEnd - 1) else -1

    if blockEnd < 0 || blockStart < 0 then None
    else
      ...
      val sourcePathOpt: Option[String] =
        ...
        if idx < 0 then None
          ...
          Some(if end < 0 then afterKey.trim else afterKey.substring(0, end).trim)

      val lines: IndexedSeq[(Int, Int, Int)] =
        val v3Idx = block.indexOf(V3Key)
        if v3Idx < 0 then IndexedSeq.empty
        else
          val afterKey = block.substring(v3Idx + V3Key.length)
          val end      = afterKey.indexOf('\n')
          val b64      = if end < 0 then afterKey.trim else afterKey.substring(0, end).trim
  ...
  def mapPosition(meta: Meta, generatedLine: Int): Option[(Int, Int)] =
    ...
    if entries.isEmpty then None
      ...
      while lo <= hi do
        val mid = (lo + hi) >>> 1
        if entries(mid)._1 <= generatedLine then
  ...
  private def decodeMappings(mappings: String): IndexedSeq[(Int, Int, Int)] =
    ...
    def readVlq(): Int =
      ...
      while continuation && i < mappings.length do
        val c = mappings(i).toInt
        if c >= 128 then i += 1 // skip non-ASCII (shouldn't occur)
        else
          val digit = B64(c)
          i += 1
          if digit < 0 then continuation = false
          else
